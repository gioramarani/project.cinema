...
function createCinema() {
    ...
    for (var i = 0; i < 6; i++) {
        ...
        for (var j = 0; j < 10; j++) {
            ...
            if (j === 7 || i === 3 || j === 2) cell.isSeat = false
            cinema[i][j] = cell
        }
    }
...
function showSeatDetails(pos) {
    ...
    elPopup.querySelector('.show-seats').innerHTML = `<button class="show-seats" onclick="showSeatsAround(${pos.i}, ${pos.j})" >🧙‍♂️</button>`
...
function showSeatsAround(idxi, idxj){
  highlightAvailableSeatsAround(gCinema, idxi, idxj)
  setTimeout(highlightAvailableSeatsAround,2500,gCinema, idxi, idxj)
}
    ...
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            if (currCell.isSeat && !currCell.isBooked) {
                    document.querySelector(`[data-i="${i}"][data-j="${j}"]`).classList.toggle('highlight')
